...
| D7 | Support domain (Phase 2) | **Smart-home device support desk, using Home Assistant's public docs corpus** — decided 2026-07-26 | Real, permissively-licensed, 1500+ documents (far above the 50+ minimum), naturally support/troubleshooting-shaped. Knowledge corpus (docs) and ticketing (Phase 3, D9) are separate concerns — we don't file against the real Home Assistant repo; a dedicated tracker repo is our own fictional support desk's ticket queue. Architecture stays domain-independent — swapping corpora is a re-run of the ingestion CLI on a different folder. |
...
| D9 | Ticketing (Phase 3) | **GitHub Issues** on a dedicated `ashutosh-rath02/voiceai-support-tickets` repo — decided 2026-07-27 | Real API, real persisted issue with a real URL; a fine-grained PAT scoped to just this one repo keeps the credential least-privilege. Simpler than Zendesk/Freshdesk account setup. |
| D10 | Scheduling (Phase 3) | **Cal.com** — decided 2026-07-27 | REST API v2 with plain API-key auth (no OAuth consent-screen setup like Google Calendar), free tier, real availability + real bookable events. |
| D11 | Messaging (Phase 3) | **Resend** (email) — decided 2026-07-27 | API-key auth, generous free tier, no domain-verification blocker for sending to a self-verified test address. WhatsApp/SMS stay optional per PRD. |
...
### Phase 3 — Real business tools (in progress)

- **3a — Customer data + read tool (done, 2026-07-27):** `customers`, `customer_contacts`, `products_or_services` (device/integration catalog matched to real ingested docs — Zigbee/Z-Wave/MQTT/Hue/Nest), `customer_products` (registered devices) tables; 4 real seeded customers, idempotent via primary-contact-email check. `lookup_customer` tool (email exact match, phone digit-normalized match for STT robustness) stamps the identified customer onto `conversations.customer_id`, surfaced in both the conversation list and replay header. New `agent_versions` row `v0.3.0` (asks for email/phone early, doesn't gate help on it) promoted ACTIVE. **Verified:** direct LLM test — correctly withheld the tool call until a contact value was actually given, then called it with the right argument; live voice call — real identification attempt exposed and led to fixing two real bugs: (1) a live-transcript UI bug where each STT-final segment (Deepgram splits continuous speech into several finals on natural pauses) rendered as a separate permanent chat bubble instead of one growing message — traced into LiveKit's own source to confirm the stable-id publishing behavior, then fixed by grouping consecutive same-speaker segments into one bubble on the frontend, mirroring what the backend's turn-detector already did correctly; (2) confirmed voice-dictated email capture is unreliable (STT renders "@" as "at the rate" etc.) — a known hard problem, not fixed, worth a future spell-back-confirmation UX.
- **3b — Tool execution + confirmation infra (done, 2026-07-27):** `tool_executions` (proposed/executing/succeeded/rejected) + `tool_confirmations` tables. Generic `propose_tool_execution` / `confirm_tool_execution` / `reject_tool_execution` in `app/agent/confirmation.py` implement PRD §6.5's ACTION_PROPOSAL → USER_CONFIRMATION → TOOL_EXECUTION flow; the LLM never chooses what code runs — it references a proposal by id, and a server-side `EXECUTORS` registry (populated by `app/services/*`, one module per real action) dispatches to the actual side effect by `tool_name`. **Idempotency has no separate key column** — `id` on `tool_executions` is the key, enforced by an atomic `UPDATE ... WHERE status = 'proposed'`; only one caller can ever win that race for a given id, so a duplicate confirm/retry always returns the already-settled outcome instead of re-executing. On an unhandled executor exception the whole transaction rolls back, reverting to `proposed` rather than a dead-end `failed` — the same confirmation can be retried without re-asking the caller. Proven with a real, permanent workflow (not a throwaway demo): confirmation-gated **customer contact-info updates** (`app/services/customers.py`), one of the PRD's explicitly named confirmation-required actions — real write to `customer_contacts`, zero external accounts needed. New `agent_versions` row `v0.4.0` promoted ACTIVE. **Verified two ways:** (1) direct state-machine tests against the real DB — propose→confirm succeeded with a real write; confirming the *same* execution id again returned the cached result with **zero** duplicate rows (exactly one `customer_contacts` row, exactly one `tool_confirmations` row); confirming a rejected or nonexistent execution was correctly refused. Caught and fixed a real bug in the process — logging `execution.tool_name` *after* `session.rollback()` crashed with SQLAlchemy `MissingGreenlet`, because rollback (unlike commit, which we run with `expire_on_commit=False`) unconditionally expires ORM objects; fixed by logging before rollback. (2) direct LLM behavior test against the real OpenAI API — an ambiguous reply ("hmm okay maybe") correctly did *not* trigger `confirm_pending_action` and instead re-asked with a spelled-out digit read-back; a clear "yes, please do it" correctly triggered it with the right reference id.
- **3c — Ticketing (GitHub Issues, needs a scoped PAT — D9):** `create_support_ticket` tool, confirmation-gated, creates a real GitHub issue and stores the returned issue number/URL on a `support_tickets` row. The PRD's required "write" workflow.
- **3d — Scheduling (Cal.com, needs an API key — D10):** `check_availability` (read) + `book_appointment` (confirmation-gated write) tools against real Cal.com availability/bookings, stored on an `appointments` row.
- **3e — Messaging (Resend, needs an API key — D11):** after a confirmed ticket or booking, send a real confirmation email summarizing what happened — the PRD's required real message-delivery integration.

Accounts needed from the user before 3c–3e can go live: a fine-grained GitHub PAT scoped to the new ticket-tracker repo, a Cal.com API key, a Resend API key. 3a/3b need nothing external and can start immediately.
