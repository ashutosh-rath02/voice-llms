...
- **3b — Tool execution + confirmation infra (no new accounts needed):** `tool_executions` / `tool_confirmations` tables; a small state machine in the orchestrator (ACTION_PROPOSAL → USER_CONFIRMATION → TOOL_EXECUTION → RESOLUTION_CONFIRMATION per PRD §6.5); idempotency-key enforcement so a repeated tool call/duplicate confirmation can't double-execute. This is the safety layer every write tool below plugs into.
